...
def update_file():
    inst = inst_update()  # Update the instance count
    print(f"Instances attempted: {inst}")  # For debugging output
    return inst  # Return the updated instance count


# Main code logic for instance check and funny window creation
inst = update_file()
            
# Print file contents for debugging
print("instances attempted:\n", inst)
         
inst = update_file()
with open(inst_file, "r"):
    
    if inst >= 4:
        create_funny_window_process()
        create_funny_windows()  # Trigger the funny windows creation
        with open(inst_file, 'w') as file:
            file.write("0")  # Reset instance count if >= 4
    else:
